...
function validateNumber (number) {
    ...
    if(isNaN(number) || (number == '')){
        ...
        const visit={
                 ...
                 cost:" 18000/person",
                 persons:number,
                 discount : (number * 18000)*0.1 + ' (10%)',
                 costForTotalPersons : (number * 18000) - (number * 18000)*0.1,
                //  totalCost: (costForTotalPersons - (costForTotalPersons*(10*0.01))),
         }
        cart.push(visit);
        console.log(cart);
        localStorage.setItem("countryD","Dubai");
        localStorage.setItem("costD",visit.cost);
        localStorage.setItem("personsD",visit.persons);
        localStorage.setItem("discountD",visit.discount);
        localStorage.setItem("costForTotalPersonsD",visit.costForTotalPersons);
